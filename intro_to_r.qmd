...
In the RStudio console, you can see the symbol below. This is the **command prompt**, indicating that the system is ready to execute an instruction or command.
...
Before going forward, note that lines of code starting with the symbol `#` are not interpreted. This is used to introduce comments in your code for readability and documentation. We will come to that later when we talk about scripts and notebooks.

::: callout-important
**Note**: This course is not intended to be an in-depth R programming course. We will use the language to illustrate the concepts in the lectures and provide you with simple examples so that you can perform basic data analysis.

There are plenty of online guided resources for further self-study of R and its capabilities. If you would like to go beyond the basics in the course or familiarize further with the language, I would suggest two starting pages:
...
A simple command is to perform an arithmetic operations. For example:
...
Variable names in R can be anything, but follow a few rules:
...
Structures or dataframes can be thought of as sets of data organized in a table format in rows and columns. They can be created using the `dataframe()` function.
...
The parameters of a function are normally called *arguments.* You can either manually specify a function’s arguments or use the function’s *default values*. In the examples above `mean()` and `sum()` are simple functions with not many arguments, but this is not normally the case.
...
As you can see above, `mean()` actually has two other arguments, `trim` and `na.rm` . The arguments have a default value, so if we don't explicitly include them when calling the function, they will use that value.

Let's look at an example using `na.rm` :
...
The function tries to calculate the average, but when one value is not available (`NA`) the result is also `NA` . In the help page for `mean()` function in the image above we see there is an argument `na.rm` that we can use to instruct the function to ignore the missing elements in the data for the calculation:

```{r}
mean(rt, na.rm = TRUE)
```

Now the function worked and summed all the numbers and divided them by 9, ignoring the missing data point in the calculation of the average.

We will work with functions a lot throughout this book and you’ll get lots of practice in understanding their behavior, so don't panic.

Finally an advanced note to make you aware that you can define your own functions. The following code defines a new function called `add()` that, adds two numbers:
