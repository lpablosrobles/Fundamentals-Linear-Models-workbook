...
You can refer to individual columns on a dataframe using the `$` operator.
...
1.  Use the operator `?` followed by the function name in the console:

    ```{r}
    ?mean
    ```

    If you type the function above the help page for the `mean()` function will open in the RStudio Help panel as in the image below:
