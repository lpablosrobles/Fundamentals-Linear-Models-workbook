...
Let's see an example of usage loading the file in the `/data` directorate "*ELP_full_length_frequency.csv"*
...
To illustrate again let's load a file in the `/data` directory containing the sample dataset from the previous exercise, saved in R format using the function below:

```{r}
data_sample_r <- readRDS("./data/sample_wg1.Rda")
data_sample_r
```
...
As an example again, let's open a file saved in SPSS from a ERP (Event Related Potential) study of code-switching in Dutch.
...
Again to preserve memory, let's clean the workspace by removing the variable we created using :
